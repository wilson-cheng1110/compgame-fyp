    ...
    return (
      <div className="min-h-screen bg-[#f9fafb] text-black flex flex-col items-center justify-start pt-10 p-6">
        ...
        <div className="w-full max-w-2xl grid md:grid-cols-2 gap-4 mb-8 font-pixelify-sans text-sm text-gray-700">
          <div className="border-2 border-black bg-white p-4"><p className="font-press-start-2p text-[#004d4d] text-[10px] mb-2">H₀ vs H₁ (Hₐ)</p><strong>H₀</strong> (null) = &ldquo;no difference&rdquo;, e.g. <em>layout B is NOT faster than A</em>. <strong>H₁</strong> (alternative, also written Hₐ) = &ldquo;there is a difference&rdquo;, e.g. <em>B IS faster</em>. You gather data and try to <strong>reject H₀</strong>.</div>
          <div className="border-2 border-black bg-white p-4"><p className="font-press-start-2p text-[#004d4d] text-[10px] mb-2">Between vs within</p>Between = different people per condition (no order effects, more variance). Within = same people all conditions (less variance, but order effects).</div>
          <div className="border-2 border-black bg-white p-4"><p className="font-press-start-2p text-[#004d4d] text-[10px] mb-2">Confound</p>An uncontrolled variable that co-varies with the IV, so you can&apos;t tell what caused the effect.</div>
          <div className="border-2 border-black bg-white p-4"><p className="font-press-start-2p text-[#004d4d] text-[10px] mb-2">Order effects</p>Practice and fatigue bias within-subjects results — controlled by counter-balancing.</div>
        </div>
        <button
  ...
  if (phase === "build") {
    ...
    return (
      <div className="min-h-screen bg-[#f9fafb] text-black flex flex-col items-center justify-start pt-8 p-6">
        <div className="bg-white border-2 border-black p-4 mb-6 max-w-xl text-center shadow-[4px_4px_0px_0px_#004d4d]">
        ...
        {step === 0 && (
          <div className="w-full max-w-xl space-y-3">
            <p className="font-press-start-2p text-[10px] text-[#004d4d] mb-1">Step 1 — choose a design</p>
        ...
        {step === 1 && (
          <div className="w-full max-w-xl space-y-3">
            <p className="font-press-start-2p text-[10px] text-[#004d4d] mb-1">Step 2 — control order effects</p>
        ...
        {step === 2 && (
          <div className="w-full max-w-xl space-y-3">
            ...
            <p className="font-press-start-2p text-[10px] text-[#004d4d] mb-1">
