...
import { ArrowLeft, Volume2, VolumeX, RefreshCw, Clock, Info, Check, Trash2 } from "lucide-react"
...
export default function RRAlgorithm() {
  ...
  const [timeQuantum] = useState(4) // Fixed time quantum for RR (q = 4)
  const [processes, setProcesses] = useState<Process[]>([
    {
      ...
      burst: 15,
      remainingTime: 15,
      imageUrl: "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/RR_A_P1-DFwnHIskOJpWEXFc7439N2F6dAjAp5.png",
    },
    {
      id: "p2",
      name: "P2",
      ...
      burst: 4,
      remainingTime: 4,
      imageUrl: "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/RR_A_P2-jb2P7Yni5I6FciGYlLpqAQ67IhVHsw.png",
    },
    {
      id: "p3",
      name: "P3",
      ...
      burst: 2,
      remainingTime: 2,
      imageUrl: "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/RR_A_P3-NI4vcluSDxY3Cg6l0sY6YpmcfH0vmX.png",
    },
  ])
  ...
  const correctAnswers = {
    p1: { completion: "21", turnaround: "21", waiting: "6", response: "0" },
    p2: { completion: "8", turnaround: "5", waiting: "1", response: "0" },
    p3: { completion: "14", turnaround: "8", waiting: "6", response: "10" },
    avgResponse: "3.33",
    avgTAT: "11.33",
  }

  // Correct Gantt Chart for RR
  const correctGanttChart = [
    { id: "p1", start: 0, end: 4 },
    { id: "p2", start: 4, end: 8 },
    { id: "p1", start: 8, end: 12 },
    { id: "p3", start: 12, end: 14 },
    { id: "p1", start: 14, end: 18 },
    { id: "p1", start: 18, end: 21 },
  ]
  ...
  const handleDropInRunning = () => {
    ...
    if (process) {
      ...
      // If it's the last process and its remaining time is less than the quantum,
      // run it for its remaining time
      ...
      if (processIndex2 !== -1) {
        updatedProcesses2[processIndex2].remainingTime -= timeToRun
  ...
  const handleSubmit = () => {
    ...
    // Play appropriate sound based on badge achievement (score >= 12)
    if (isSoundOn) {
      try {
        if (finalScore >= 15) {
            ...
            <div className="col-span-1 bg-[#441dbf] p-4 rounded-none">
              ...
              <div className="mb-4">
                ...
                {showTips && (
                  <p className="font-pixelify-sans text-white text-sm md:text-base">
                    Each process gets a fixed time slice (quantum: q). After using its time slice, a process moves to
                    the back of the queue if it is not yet completed. Arrange the processes in the correct execution
                    order based on Round Robin scheduling.
                  </p>
                )}
              </div>
              ...
              {hasSubmitted && score !== null && (
                <div className="mt-4 bg-black bg-opacity-70 p-3 rounded-md">
                  <p className={`font-pixelify-sans text-lg ${score >= 12 ? "text-green-500" : "text-red-500"}`}>
                    Score: {score}/15
                  </p>

                  {/* Error message based on what was wrong */}
                  {score < 15 && getErrorMessage() && (
                    <p className={`font-pixelify-sans text-sm mt-2 ${score >= 12 ? "text-green-500" : "text-red-500"}`}>
                      {getErrorMessage()}
                    </p>
                  )}
                ...
                <table className="w-full text-white mb-4">
                  <thead>
                    <tr className="font-pixelify-sans text-[#4cf190]">
                      <th className="text-left p-2" colSpan={7}>
                        Given: q = {timeQuantum}
                      </th>
                    </tr>
                    <tr className="font-pixelify-sans text-[#4cf190]">
          ...
          <div className="bg-black border-2 border-[#441dbf] p-4 rounded-none">
            <div className="font-pixelify-sans text-[#4cf190] text-sm space-y-2">
              ...
              <p className="text-xs text-white italic">
                *Please note that all answers are rounded to two decimal places.
              </p>
            </div>
          </div>
