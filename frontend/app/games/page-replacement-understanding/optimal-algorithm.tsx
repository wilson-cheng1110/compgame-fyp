...
export default function OptimalAlgorithm() {
  ...
  // Page reference sequence for Optimal algorithm
  const pageReferences = [0, 1, 3, 2, 4, 0, 2, 4]

  // Table data to match Figma exactly for Optimal algorithm
  const [tableData, setTableData] = useState<DisplayCell[][][]>([
    ...
    [
      ...
      [{ value: "0", color: "text-black", isVisible: false }], // Column 10
      [{ value: "0", color: "text-black", isVisible: false }], // Column 11
      [{ value: "0", color: "text-blue-600", isVisible: false }], // Column 12
    ],
    // F2 row
    [
      ...
      [{ value: "4", color: "text-black", isVisible: true }],
      [{ value: "4", color: "text-blue-600", isVisible: true }],
      [{ value: "1*", color: "text-red-600", isVisible: false }], // Column 9
      [{ value: "1", color: "text-black", isVisible: false }], // Column 10
      [{ value: "1", color: "text-blue-600", isVisible: false }], // Column 11
      [{ value: "1", color: "text-black", isVisible: false }], // Column 12
    ],
    // F3 row
    [
      [{ value: "-", color: "text-black", isVisible: true }],
      ...
      [{ value: "2", color: "text-black", isVisible: true }],
      [{ value: "2", color: "text-blue-600", isVisible: true }],
      [{ value: "2", color: "text-black", isVisible: true }],
      [{ value: "2", color: "text-black", isVisible: false }], // Column 9
  ...
  const [prompt, setPrompt] = useState("Drag the cars to the correct frames according to Optimal rules.")
  ...
  const updateTableData = (carId: string, frameId: string) => {
    ...
    if (carId === "car1") {
      // Red car (page 1) in F2
      // Make the 9th column (index 8) visible for all frames
      newTableData[0][8][0].isVisible = true // Show 0 in F1
      newTableData[1][8][0].isVisible = true // Show 1* in F2 (red with asterisk)
      newTableData[2][8][0].isVisible = true // Show 2 in F3
    } else if (carId === "car2") {
      // Green car (page 3) in F3
      // Make the 10th column (index 9) visible
      newTableData[0][9][0].isVisible = true // Show 0 in F1
      newTableData[1][9][0].isVisible = true // Show 1 in F2
      newTableData[2][9][0].isVisible = true // Show 3* in F3 (red with asterisk)
    } else if (carId === "car3") {
      // Orange car (page 1) in F2
      // Make the 11th column (index 10) visible
      newTableData[0][10][0].isVisible = true // Show 0 in F1
      newTableData[1][10][0].isVisible = true // Show 1 in F2 (blue)
      newTableData[2][10][0].isVisible = true // Show 3 in F3
    } else if (carId === "car4") {
      // Brown car (page 0) in F1
      // Make the 12th column (index 11) visible
      newTableData[0][11][0].isVisible = true // Show 0 in F1 (blue)
      newTableData[1][11][0].isVisible = true // Show 1 in F2
      newTableData[2][11][0].isVisible = true // Show 3 in F3
    }
  ...
  const handleDrop = (e: React.DragEvent, frameId: string) => {
    ...
    // Check if this is the correct frame based on Optimal rules
    // Updated mapping based on the provided correct mapping
    const correctFrameMapping: Record<string, string> = {
      car1: "frame2", // Red car (page 1) goes to F2
      car2: "frame3", // Green car (page 3) goes to F3
      car3: "frame2", // Orange car (page 1) goes to F2
      car4: "frame1", // Brown car (page 0) goes to F1
    }

    if (frameId !== correctFrameMapping[carId]) {
      // Incorrect frame
      setPrompt(
        "Incorrect! Choose the page whose next use is farthest in the future. If there are multiple pages that will not be used for the longest time in the future, choose a smaller frame.",
      )
    ...
    if (currentStep + 1 >= 4) {
      ...
      setPrompt("Congratulations! You've completed the Optimal page replacement algorithm.")
  ...
  // Restart the game
  const restartGame = () => {
    // This will reload the current page, giving the same effect as clicking Optimal on the homepage
    window.location.href = "/optimal"
  }
      ...
      <div className="relative z-10 min-h-[800px] flex items-start justify-center p-4">
        <div className="bg-[#b4dcd7] w-full max-w-5xl p-8">
          ...
          <div className="bg-[#0d453b] p-4 mb-4">
            <h1 className="font-pixelify-sans text-[#4cf190] text-xl md:text-2xl mb-2">Optimal Page Replacement</h1>
            <p className="font-pixelify-sans text-white text-sm md:text-base">
              Use the Optimal page replacement strategy. When a page fault occurs, replace the page that will not be
              used for the longest time in the future. Look ahead in the reference string to decide which page to
              replace. Click and drag each car into the parking space according to Optimal rules.
            </p>
          </div>
