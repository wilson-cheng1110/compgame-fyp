...
print("\n-- session --")
r = c.post("/api/auth/session", json={"sid": "24012345D"})
...
r = c.post("/api/auth/session", json={"sid": "99999999Q"})
...
print("\n-- withdrawal --")
c.post("/api/auth/session", json={"sid": "24067890X"})
r = c.post("/api/auth/withdraw")
check("withdraw ok", r.status_code == 200, r.json())
c.cookies.clear()
r = c.post("/api/auth/session", json={"sid": "24067890X"})
check("withdrawn SID can't re-enter", r.status_code == 403, r.status_code)
