...
(async () => {
  ...
  try {
    ...
    console.log('WAITING FOR RESPONSE (30 seconds max)...');
    console.log('Progress: ', { end: false });
    ...
    for (let i = 0; i < 60; i++) {
      ...
      // Take snapshots
      if (i === 6) {
        let ssPath = path.join(screenshotDir, 'RESPONSE-At-3sec.png');
        await page.screenshot({ path: ssPath });
      }
      if (i === 14) {
        let ssPath = path.join(screenshotDir, 'RESPONSE-At-7sec.png');
        await page.screenshot({ path: ssPath });
      }
      if (i === 30) {
        let ssPath = path.join(screenshotDir, 'RESPONSE-At-15sec.png');
        await page.screenshot({ path: ssPath });
      }
    }
    ...
    console.log(`Saved: FINAL-RESPONSE-COMPLETE.png\n`);
    process.exit(0);
